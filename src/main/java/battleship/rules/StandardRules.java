...
public class StandardRules implements Rules {
    ...
    private static Optional<Turn> createTurn(TurnAction action, String label) {
        return Optional.of(new Turn(action, String.format(StandardRules.PROMPT_PATTERN, label)));
    }
    ...
    public Optional<Turn> getNextTurn(final Game game) {
        ...
        switch (eventCount) {
            case 0:
                return createTurn(new ShipPlacementAction(player, ShipType.CARRIER), "Flugzeugträgers");
            case 1:
                return createTurn(new ShipPlacementAction(player, ShipType.BATTLESHIP), "Schlachtschiffs");
            case 2:
                return createTurn(new ShipPlacementAction(player, ShipType.CRUISER), "Kreuzers");
            case 3:
                return createTurn(new ShipPlacementAction(player, ShipType.DESTROYER), "Zerstörers");
            case 4:
                return createTurn(new ShipPlacementAction(player, ShipType.CANNON_BOAT), "Kanonenboots");
            default:
                return createTurn(new ShotAction(player), "nächsten Schusses");
        }
    }
